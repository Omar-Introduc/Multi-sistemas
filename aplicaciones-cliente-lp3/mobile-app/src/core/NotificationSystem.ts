...
export class NotificationSystem {
  ...
  private maxVisibleNotifications = 3;
  ...
  public show(notification: Omit<NotificationConfig, 'id'>): string {
    ...
    const fullNotification: NotificationConfig = {
      ...
      vibration: notification.vibration ?? true,
  ...
  /**
   * Muestra notificación usando Alert nativo de React Native
   */
  public alert(title: string, message: string, actions?: { text: string; style?: 'default' | 'cancel' | 'destructive'; onPress?: () => void }[]): void {
  ...
  private async processDisplayQueue(): Promise<void> {
    ...
    while (this.displayQueue.length > 0) {
      ...
      // Usar Alert para notificaciones críticas
      if (notification.type === 'error' || notification.persistent) {
        this.alert(notification.title, notification.message, [
          {
            text: 'OK',
            onPress: () => this.hide(notification.id)
          },
        ...
        // Para notificaciones normales, usar Alert simple
        this.alert(notification.title, notification.message);
      }
  ...
  private getNotificationTemplates(): Record<string, any> {
    return {
      ...
      // Template para sincronización
      syncComplete: {
        type: 'success',
        title: 'Sincronización Completada',
        message: 'Tus datos han sido sincronizados exitosamente.',
        sound: true
      },
      
      // Template para modo offline
      offlineMode: {
        type: 'warning',
        title: 'Modo Sin Conexión',
        message: 'Estás trabajando sin conexión. Los cambios se sincronizarán cuando恢复es la conexión.',
        persistent: true
      }
    };
  }
  ...
  public showProgress(title: string, message: string, progress: number): string {
    ...
    // Esta función se puede llamar externamente para actualizar el progreso
  ...
  /**
   * Actualiza el progreso de una notificación
   */
  public updateProgress(notificationId: string, progress: number): void {
    const notification = this.notifications.get(notificationId);
    if (notification && (this as any).updateProgress) {
      (this as any).updateProgress(progress);
    }
  }

  /**
   * Persiste una notificación en AsyncStorage
   */
  private async persistNotification(notification: NotificationConfig): Promise<void> {
    try {
      const persisted = await this.getPersistedNotifications();
      persisted[notification.id] = notification;
      await AsyncStorage.setItem('notification_system', JSON.stringify(persisted));
    } catch (error) {
   ...
   */
  private async unpersistNotification(id: string): Promise<void> {
    try {
      const persisted = await this.getPersistedNotifications();
      delete persisted[id];
      await AsyncStorage.setItem('notification_system', JSON.stringify(persisted));
   ...
   */
  private async getPersistedNotifications(): Promise<Record<string, NotificationConfig>> {
    try {
      const stored = await AsyncStorage.getItem('notification_system');
   ...
   */
  public async clearPersistedNotifications(): Promise<void> {
    await AsyncStorage.removeItem('notification_system');
