...
Sistema distribuido para entrenamiento y reconocimiento de objetos/personas utilizando modelos de IA (XGBoost), comunicación vía Sockets, y despliegue en cluster.
...
- **Descripción:** Desarrollar una librería o módulo compartido para manejar la comunicación por Sockets TCP/IP.
- **Estado:** Completado.
...
### Issues
#### 2.1. Diseño del Modelo de IA (Completado)
- **Descripción:** Definir y programar el modelo de IA a utilizar. Se ha seleccionado **XGBoost** sobre HOG features tras analizar resultados experimentales.
- **Tareas:**
    - Implementar pipeline de pre-procesamiento (HOG).
    - Integrar librería XGBoost.
    - Validar resultados de precisión y rendimiento.
...
- **Tareas:**
    ...
    - Implementar arquitectura Master-Worker para cálculo distribuido de características.
...
- **Tareas:**
    - Serialización del modelo XGBoost y mapa de etiquetas.
...
- **Tareas:**
    ...
    - Carga dinámica del modelo XGBoost en memoria.
...
- **Tareas:**
    ...
    - Ejecutar inferencia (Forward pass) usando XGBoost.
    - Filtrar resultados por umbral de confianza (Softmax probability).
...
- **Validación Técnica:** Asegurar que las restricciones (No Frameworks de alto nivel para comunicación, Solo Sockets) se respetan en cada issue. XGBoost es permitido como librería de modelo interno.
