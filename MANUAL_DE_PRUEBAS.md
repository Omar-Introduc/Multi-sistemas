...
Este documento describe los pasos para ejecutar el sistema con **Inteligencia Real (HOG+XGBoost)** y **Entrenamiento Distribuido**.

## Requisitos Previos
- Python 3.8+
- Librerías: `opencv-python`, `numpy`, `xgboost`, `scikit-learn`
...
Dataset/
  ...
  └── Auto/
      ...
      └── img2.jpg
```
*Salida esperada:* `Distributing X samples to 1 workers...` -> `Aggregation complete` -> `Training XGBoost model...` -> `TRAIN_COMPLETE`.
...
- **Predicciones "Unknown":** El modelo XGBoost requiere características consistentes. Asegúrate de que las imágenes de entrenamiento cubran variaciones de ángulo e iluminación.
- **Error de librería:** Si obtienes errores de importación de `xgboost`, asegúrate de haber ejecutado `pip install -r requirements.txt`.
